    ...
    try {
      ...
      if (data.status === 'success') {
        try {
          ...
          // 存储穿着建议图片
          if (suitPicturesResult && suitPicturesResult.status === 'success') {
            sessionStorage.setItem(
              'suitPictures',
              JSON.stringify(suitPicturesResult)
            );
            console.log('穿着建议图片已存储到sessionStorage');
          }
        ...
        console.error('获取个性化分析数据失败:', data);
        // push到step1
